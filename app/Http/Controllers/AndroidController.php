...
class AndroidController extends Controller
{
    ...
    public function getDanhSach()
    {
        ...
        return view('admin.theloai.android', ['theloai' => $theloai, 'status' => $status, 'pro_file' => $pro_file]);
    }
    //-----------------------------------------------------------------------------------------------//
    ...
    public function getdelete($id)
    {
        $theloai = Email::find($id);
        $theloai->delete();
        return redirect('admin/Android/danhsach')->with('thongbao', 'xóa thành công');
    }
    //-----------------------------------------------------------------------------------------------//
    ...
    public function delete_all($id_mail_profile)
    {
        if ($id_mail_profile == "All") {
            Email::truncate();
            return redirect('admin/Android/' . $id_mail_profile)->with('thongbao', 'xóa thành công');
        } else {
    ...
    public function delete_all_serch(Request $request)
    {
        ...
        return redirect('admin/Android/All')->with('thongbao', 'xóa thành công');
    }
    //-----------------------------------------------------------------------------------------------//
    ...
    public function search(Request $request)
    {
        ...
        if ($request) {
            ...
            if ($download_book != "All" and $download_book != "Mail lỗi") {
                $download_book = Email::where([['download_book', 'like', $download_book], ['id_pm', 'like', $mail_profile]])->get();
                $id_download_book = [];
            ...
            if ($download_book == "Mail lỗi") {
                $download_book = Email::where([['download_book', 'like', '%Mail%'], ['id_pm', 'like', $mail_profile]])->get();
                $id_download_book = [];
                foreach ($download_book as $value) {
                    $id_download_book[] = $value->id;
                }
            ...
            if ($send_mail != "All" and $send_mail != "Mail lỗi") {
                $send_mail = Email::where([['send_mail', 'like', $send_mail], ['id_pm', 'like', $mail_profile]])->get();
                $id_send_mail = [];
            ...
            if ($send_mail == "Mail lỗi") {
                $send_mail = Email::where([['send_mail', 'like', '%Mail%'], ['id_pm', 'like', $mail_profile]])->get();
                $id_send_mail = [];
                foreach ($send_mail as $value) {
                    $id_send_mail[] = $value->id;
                }
            ...
            if ($orderby == 'loai_tru') {
                ...
                //var_dump($theloai);
                return view('admin.theloai.searchAndroid', ['theloai' => $theloai, 'serch' => $request, 'status' => $status, 'id_mail_profile' => $id_mail_profile, 'pro_file' => $pro_file]);
            } else {
